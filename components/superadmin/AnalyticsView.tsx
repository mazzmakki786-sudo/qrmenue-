...
type AnalyticsData = {
  ...
  last7: { total: number; revenue: number; cancelled: number; byDay: any[]; byStatus: any; byType: any; recent: any[] }
  last30: any
  customerFlow: {
    ...
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => <div key={i} className="h-24 bg-white rounded-xl border border-[#F0F0F0] animate-pulse" />)}
        </div>
        <div className="h-64 bg-white rounded-xl border border-[#F0F0F0] animate-pulse" />
      </div>
    )
  }

  if (!data) {
    return <div className="bg-white rounded-xl border border-[#F0F0F0] p-12 text-center"><p className="text-[#999] text-sm">Failed to load analytics</p></div>
  }
          ...
          {data.restaurants.byDay.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={120}>
              <BarChart data={data.restaurants.byDay}>
                ...
                <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8 }} />
                <Bar dataKey="count" fill="#25D366" radius={[2, 2, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
