    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-5 py-20 md:py-28 text-center">
          <div className="inline-flex px-3 py-1 rounded-full bg-[#F0F0F0] text-xs font-medium text-[#555] mb-6">
            Live in Pakistan
          </div>
      ...
      <section className="max-w-6xl mx-auto px-5 py-16 md:py-24">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-8">
          {[
            ...
            <div key={s.step} className={`text-center p-4 md:p-6 rounded-xl md:rounded-2xl border border-[#F0F0F0] hover:border-black/20 transition-colors ${i === 2 ? "col-span-2 md:col-span-1" : ""}`}>
              <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-black text-white text-[11px] md:text-sm font-bold flex items-center justify-center mx-auto mb-3 md:mb-4">
                {s.step}
              </div>
              <h3 className="text-sm md:text-base font-semibold mb-1 md:mb-2">{s.title}</h3>
              <p className="text-[11px] md:text-sm text-[#888] leading-relaxed">{s.desc}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 md:gap-6">
            {plans.map((plan) => (
              <div
                key={plan.key}
                className={`relative rounded-xl md:rounded-2xl border p-2.5 md:p-6 flex flex-col bg-white transition-all hover:shadow-lg ${
                  plan.popular ? "border-black shadow-md ring-1 ring-black/5" : "border-[#E8E8E8]"
                }`}
              >
                {plan.badge && (
                  <span className="absolute -top-2 left-1/2 -translate-x-1/2 text-[8px] md:text-[11px] font-semibold bg-black text-white px-1.5 md:px-3 py-0.5 md:py-1 rounded-full whitespace-nowrap">
                    {plan.badge}
                  </span>
                )}
                <div className="mb-2 md:mb-6">
                  ...
                  <div className="mt-0.5 md:mt-2 flex items-baseline gap-0.5 md:gap-1">
                    ...
                    <span className="text-[9px] md:text-sm text-[#888]">/{plan.period}</span>
                  </div>
                </div>
