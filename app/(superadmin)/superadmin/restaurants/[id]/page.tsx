...
export default function RestaurantDetailPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "restaurants", filter: `id=eq.${id}` },
        () => scheduleRefetch()
      )
      .on(
        "postgres_changes",
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "dishes", filter: `restaurant_id=eq.${id}` },
        () => scheduleRefetch()
      )
      .subscribe()
    return () => {
  ...
  const handlePlanUpdate = async (plan: string, endDate: string) => {
    ...
    try {
      await fetch(`/api/superadmin/restaurants/${id}/toggle`, {
        ...
        body: JSON.stringify({
          plan,
          plan_end_date: plan === "trial" ? null : endDate || null,
        }),
      })
  ...
  const handleResetTrial = async () => {
    if (!restaurant) return
    if (!window.confirm("Reset trial for this restaurant? This will set plan=trial with a fresh 7-day window and clear sent reminder emails.")) {
      return
    }
    setResetting(true)
    setResetMsg(null)
    try {
      const res = await fetch(`/api/superadmin/restaurants/${id}/reset-trial`, {
        method: "POST",
      })
  ...
  const trialDaysLeft = restaurant.trial_end
    ? Math.max(0, Math.ceil((new Date(restaurant.trial_end).getTime() - Date.now()) / (24 * 60 * 60 * 1000)))
    : 0

  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-3">
        <Link href="/superadmin" className="p-2 hover:bg-white rounded-lg transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        >
          ...
          {restaurant.is_active ? (
            ...
            <ToggleLeft className="w-10 h-10 text-[#999]" />
          )}
        </button>
        <button
        ...
        >
          ...
          {restaurant.image_upload_allowed ? (
            <ToggleRight className="w-10 h-10 text-[#16A34A]" />
          ) : (
            <ToggleLeft className="w-10 h-10 text-[#999]" />
          )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Stat icon={<TrendingUp className="w-4 h-4" />} label="Total Revenue" value={formatPrice(stats.revenue)} color="green" />
      ...
      <div>
        ...
        <div className="bg-white rounded-[14px] border border-[#E8E8E8] p-4 mb-3">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-semibold">Last 7 Days</p>
            <p className="text-xs text-[#999]">{stats.last7} orders • {formatPrice(stats.last7Revenue)}</p>
          </div>
          <MiniBarChart days={chart7} />
        </div>
        <div className="bg-white rounded-[14px] border border-[#E8E8E8] p-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-semibold">Last 30 Days</p>
            <p className="text-xs text-[#999]">{stats.last30} orders • {formatPrice(stats.last30Revenue)}</p>
          </div>
          <MiniBarChart days={chart30} />
        </div>
      </div>
      ...
      <div className="bg-white rounded-[14px] border border-[#E8E8E8] p-5 space-y-3">
        ...
        {restaurant.plan === "trial" && (
          <div className="flex items-center justify-between text-sm">
            <span className="text-[#555] flex items-center gap-1.5">
              <Clock className="w-3.5 h-3.5" /> Trial ends
            </span>
        ...
        {restaurant.plan_end_date && (
          <Row
            ...
            label="Plan Ends"
            value={new Date(restaurant.plan_end_date).toLocaleDateString()}
          />
        )}
        <Row
          icon={<Calendar className="w-4 h-4" />}
      ...
      <div className="bg-white rounded-[14px] border border-[#E8E8E8] p-5">
        <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
          <RotateCcw className="w-4 h-4" /> Trial Controls
        </h3>
        <p className="text-xs text-[#555] mb-3">
          Reset this restaurant's trial to a fresh 7-day window. All sent reminder emails will be cleared, allowing the reminder sequence to fire again.
        </p>
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <Button
            ...
            disabled={resetting}
            className="!bg-[#FFF7ED] !text-[#9A3412] hover:!bg-[#FFEDD5]"
          >
            {resetting ? (
          ...
          {resetMsg && (
            <p className={`text-xs ${resetMsg.type === "ok" ? "text-[#16A34A]" : "text-[#DC2626]"}`}>
              {resetMsg.text}
            </p>
          )}
        </div>
      ...
      {customers.length > 0 && (
        <div className="bg-white rounded-[14px] border border-[#E8E8E8] p-5">
          ...
          <div className="space-y-1">
            ...
            {customers.length > 20 && (
              <p className="text-xs text-[#999] pt-2">+{customers.length - 20} more</p>
            )}
          </div>
        </div>
      )}
...
function MiniBarChart({ days }: { days: { date: string; orders: number; revenue: number }[] }) {
  if (days.length === 0) {
    return <p className="text-xs text-[#999] text-center py-4">No data</p>
  }
      ...
      {days.map((d) => (
        <div key={d.date} className="flex-1 flex flex-col items-center gap-1 min-w-0">
          <div className="w-full flex flex-col items-center justify-end h-28">
            <div
              className="w-full bg-black rounded-t min-h-[2px] hover:bg-[#333]"
              style={{ height: `${(d.orders / max) * 100}%` }}
              title={`${d.date}: ${d.orders} orders, ${formatPrice(d.revenue)}`}
            />
          </div>
          <p className="text-[9px] text-[#999] truncate w-full text-center">
