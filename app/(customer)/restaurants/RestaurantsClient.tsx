...
function RestaurantCard({
  ...
  return (
    <div className="flex items-center justify-between gap-3 p-4 sm:p-5 bg-white border border-gray-200 rounded-xl hover:border-black/30 hover:shadow-sm transition-all duration-200 group">
      ...
      <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
        <button
          ...
          className="w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center hover:bg-gray-100 transition-colors"
          aria-label={isFav ? "Remove from favorites" : "Add to favorites"}
        >
          <svg
            className={`w-5 h-5 sm:w-5.5 sm:h-5.5 transition-colors ${isFav ? "text-yellow-500" : "text-gray-300 hover:text-yellow-400"}`}
            viewBox="0 0 24 24"
        ...
        <Link
          ...
          className="text-xs font-medium text-black/60 hover:text-black hover:underline decoration-1 underline-offset-2 whitespace-nowrap transition-colors"
        >
          View Menu
        </Link>
      </div>
    ...
    <div className="min-h-screen bg-[#f9f9f9]">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 pb-24">
        ...
        <section className="mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-4xl font-bold text-black mb-4 sm:mb-6 tracking-tight">
            Browse Restaurants
          </h1>
          ...
          <div className="max-w-xl">
            <div className="relative group">
              <svg
                className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-gray-400 group-focus-within:text-black transition-colors"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={2}
              ...
              <input
                ...
                className="w-full pl-10 sm:pl-12 pr-10 py-2.5 sm:py-3 bg-white border border-gray-200 rounded-xl focus:ring-1 focus:ring-black/20 focus:border-black/30 text-sm outline-none transition-all placeholder:text-gray-400"
              />
              {searchQuery && (
                <button
            ...
            {Array.from(groupedByCity.entries()).map(([city, items]) => (
              <section key={city}>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="text-[10px] text-gray-400 font-medium tabular-nums">{items.length} {items.length === 1 ? "place" : "places"}</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                  {items.map((r) => (
                    <RestaurantCard
